use super::Module;
use super::{Function, FunctionData, Place, RegisterAllocation, Location, Instruction, 
            BinaryOp, UnaryOp, IntPredicate};
use crate::ir;
use std::collections::HashMap;

pub(super) trait Backend {
    fn new(ir: &ir::Module) -> Self;
    fn generate_instruction(&mut self, function: Function, data: &FunctionData);
    fn finalize(self) -> (Vec<u8>, HashMap<Function, (usize, usize)>);
}
...
impl<'a> X86Backend<'a> {
    ...
    fn generate(&self, asm: &mut Assembler, function: Function, data: &FunctionData) {
    ...
    pub fn new(ir: &'a Module) -> Self {
        use super::execmem::ExecutableMemory;
        ...
        for (function, data) in &backend.ir.functions {
            backend.generate(&mut asm, *function, data);
        }

        let bytes  = asm.into_bytes();
        let memory = ExecutableMemory::from_buffer(&bytes);

        std::fs::write("asm_dump.bin", &bytes);

        let mut buffer = [1u8, 2u8, 3u8, 0u8];
        
        type Func = extern "win64" fn(*mut u8) -> i32;

        let func   = unsafe { *(&memory.as_ptr() as *const _ as *const Func) };
        let result = unsafe {
